  ...
  try {
    ...
    const documents = await Promise.all(
      files.items.map(async (file) => {
        try {
          // Pour socials.json et description.txt
          if (file.name === 'socials.json' || file.name === 'description.txt') {
            const bytes = await getBytes(file);
            const text = new TextDecoder().decode(bytes);
            return {
              name: file.name,
              content: file.name === 'socials.json' ? JSON.parse(text) : text
            };
          }
          
          // Pour tous les autres fichiers (PDF, images, etc.)
          const url = await getDownloadURL(file);
          return { name: file.name, url, content: null };
